...
class NNPolicy():
    ...
    def memorize(self):
        """
        Placeholder for memory function.
        """
        raise NotImplementedError

    def choose_greedy_action(self, state: torch.Tensor) -> torch.Tensor:
        """
        Chooses action greedily based on current state.
    ...
    def choose_action(self, state: torch.Tensor) -> torch.Tensor:
        """
        Chooses action based on current state.
    ...
    def save_policy(self, policy_save_path):
        ...
        Args:
            policy_save_path: Path to save policy.
        """
        raise NotImplementedError
    ...
    def hard_update(self, target_net, behaviour_net):
        ...
        Args:
            ...
            behaviour_net: Behavior network.
        """
...
class IQLPolicy(NNPolicy):
    def __init__(self,n_observations:int , action_space:list, hyper_params:dict,load_save,id) -> None:
        ...
        if self.load_save:
            print("LOADING SAVE")
            policy_param = torch.load(self.policy_save_path, map_location=device)
...
class MADDPGPolicy(NNPolicy):
    def __init__(self, n_observations: int, action_space: list, hyper_params: dict, n_agents: int, load_save, id) -> None:
        """
        Initializes the Multi-Agent Deep Deterministic Policy Gradient (MADDPG) policy.
        ...
        if self.load_save:
            ...
            actor_params = torch.load(self.policy_save_path + "_actor.pt", map_location=device)
    ...
    def stop_noise(self):
        """
        Stops adding noise for exploration.
        """
    ...
    def choose_action(self, state_tensor: Tensor) -> Tensor:
        """
        Chooses action based on current state tensor.

        Args:
            state_tensor (Tensor): Current state tensor.

        Returns:
            Tensor: Chosen action tensor.
        """
    ...
    def optimize(self, batch):
        """
        Optimizes the policy based on the given batch.
    ...
    def save_policy(self):
        """
        Saves the policy and its related data.
        """
